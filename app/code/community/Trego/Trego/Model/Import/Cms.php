...
class Trego_Trego_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/community/Trego/Trego/etc/import/';
    }
	...
	public function importCms($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('trego')->__("Can't get the data file for import cms blocks/pages: %s", $xmlPath)
                );
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('trego')->__('%s item(s) was(were) imported.', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('trego')->__('No items were imported')
				);
			}
