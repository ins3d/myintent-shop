...
class Trego_Ajaxfilter_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price 
{
	...
	*/
	public function __construct(){
		...
		$this->_imagePath = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'wysiwyg/trego/ajaxfilter/';
	...
	*/
	...
	public function getPriceDisplayType(){
		...
		if($this->isTextBoxEnabled()){
			$html = '
				<div class="text-box">
					'.$this->getCurrencySymbol().' <input type="text" name="min" id="minPrice" class="priceTextBox minPrice" value="'.$this->getCurrMinPrice().'" style="'.$textBoxStyle.'" /> - 
					'.$this->getCurrencySymbol().' <input type="text" name="max" id="maxPrice" class="priceTextBox maxPrice" value="'.$this->getCurrMaxPrice().'" style="'.$textBoxStyle.'" />
					<input type="button" value="'.$this->getGoBtnText().'" name="go" class="go" style="'.$goBtnStyle.'" />
					<input type="hidden" id="amount" readonly="readonly" class="price-amount" style="background:none; border:none;" value="'.$this->getCurrencySymbol().$this->getCurrMinPrice()." - ".$this->getCurrencySymbol().$this->getCurrMaxPrice().'" />
	...
	public function getHtml(){
		...
		if($this->getSliderStatus()){
			$text='<div class="price">
                <label>'.$this->__('Price').'</label>
				<div>
					'.$this->getPriceDisplayType().'
					<div class="slider-range"></div>
					
				</div></div>'.$this->getSliderJs();	
