<?php
class Webtex_OnePageCheckout_Model_Type_Onepage
{
    /**
    ...
    public function initCheckout()
    {
        ...
        $shippingAddressId = $this->getQuote()->getShippingAddress()->getAddressId();
    ...
    public function saveShipping($data, $customerAddressId)
    {
        ...
        if (!empty($customerAddressId)) {
            ...
            //if ($customer) {
            //    $this->getQuote()->assignCustomerWithAddressChange($customer, $this->getQuote()->getBillingAddress(),$address);
            //}
        }
    ...
    {
        $shipping = Mage::app()->getRequest()->getPost('shipping',array());
        $countryId = isset($shipping['country_id']) ? $shipping['country_id'] : $this->getQuote()->getShippingAddress()->getCountryId();
        $request = Mage::getModel('shipping/rate_request');
        $request->setDestCountryId($countryId);
        $request->setDestRegionId($this->getQuote()->getShippingAddress()->getRegionId());
        /**
         * need to call getStreet with -1
         * to get data in string instead of array
         */
        $request->setDestStreet($this->getQuote()->getShippingAddress()->getStreet());
        $request->setDestCity($this->getQuote()->getShippingAddress()->getCity());
        $request->setDestPostcode($this->getQuote()->getShippingAddress()->getPostcode());

        $request->setStoreId($this->getQuote()->getStore()->getId());
        $request->setWebsiteId($this->getQuote()->getStore()->getWebsiteId());
        $request->setFreeShipping($this->getQuote()->getShippingAddress()->getFreeShipping());
        /**
         * Currencies need to convert in free shipping
         */
        $request->setBaseCurrency($this->getQuote()->getStore()->getBaseCurrency());
        $request->setPackageCurrency($this->getQuote()->getStore()->getCurrentCurrency());
        $request->setLimitCarrier($this->getQuote()->getShippingAddress()->getLimitCarrier());

        $request->setBaseSubtotalInclTax($this->getQuote()->getShippingAddress()->getBaseSubtotalInclTax());
        ...
        if ($result) {
            ...
            foreach ($shippingRates as $rate) {
                if($price = Mage::getStoreConfig('carriers/'.$rate->getCarrier().'/price')){
                   $rate->setPrice($price);
                }
                $rate->setCode($rate->getCarrier() . '_' . $rate->getCarrier());
                $sRates[$rate->getCarrier()][] = $rate;
                $sRates[$rate->getCarrier()][0]->carrier_sort_order = $rate->getSortOrder();
            }
        }
        uasort($sRates, array($this, '_sortRates'));
        return $sRates;
    }
