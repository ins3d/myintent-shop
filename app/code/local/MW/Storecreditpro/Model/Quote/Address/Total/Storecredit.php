...
class MW_Storecreditpro_Model_Quote_Address_Total_Storecredit extends Mage_Sales_Model_Quote_Address_Total_Abstract {	
	...
	public function collect(Mage_Sales_Model_Quote_Address $address) {
		...
		if ($customer_id) {
			...
			if ($quote->getMwStorecredit() > $baseGrandTotal && $baseGrandTotal > 0) $quote->setMwStorecredit($baseGrandTotal) ->save();
			...
			if ($quote->getMwStorecredit() > $credit_max_checkout) $quote->setMwStorecredit($credit_max_checkout) ->save();

			if($quote->getMwStorecredit() < $credit_min_checkout) $quote->setMwStorecredit(0)->save();
		...
		$quote->setMwStorecreditCheckoutMax($credit_max_checkout)->save();
		$quote->setMwStorecreditCheckoutMin($credit_min_checkout)->save();
		$quote->setMwStorecreditBuyCredit($mw_credit_buy)->save();
		$quote->setMwStorecreditDiscount($mw_storecredit)->save();
		$quote->setMwStorecreditDiscountShow($mw_storecredit_discount_show)->save();
	...
	public function fetch(Mage_Sales_Model_Quote_Address $address) {
       ...
       $credit = $address->getMwStorecredit();
        ...
        if ($amount!=0) {
        	$title = Mage::helper('storecreditpro')->__('You Redeem');
