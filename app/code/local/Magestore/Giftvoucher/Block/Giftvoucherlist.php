...
class Magestore_Giftvoucher_Block_Giftvoucherlist extends Mage_Core_Block_Template {
    ...
    public function getCodeTxt($row) {
        $input = '<input style="width:auto;" id="input-gift-code' . $row->getId() . '" readonly type="text" class="input-text" value="' . $row->getGiftCode() . '" onblur="hiddencode' . $row->getId() . '(this);">';
    ...
    public function getActions($row) {
        ...
        if ($row->getStatus() < Magestore_Giftvoucher_Model_Status::STATUS_DISABLED) {
            //Hai.Tran
            $action .= ' | <a href="javascript:void(0);" onclick="window.open(\'' . $this->getUrl('*/*/print', array('id' => $row->getId())) . '\',\'newWindow\', \'width=1000,height=700,resizable=yes,scrollbars=yes\')" >' . $this->__('Print') . '</a>';
            if ($row->getRecipientName() && $row->getRecipientEmail() && ($row->getCustomerId() == Mage::getSingleton('customer/session')->getCustomerId() || $row->getCustomerEmail() == Mage::getSingleton('customer/session')->getCustomer()->getEmail())
                ...
                $action .= ' | <a href="' . $this->getUrl('*/*/email', array('id' => $row->getId())) . '">' . $this->__('Email') . '</a>';
            }
        }
        // 
