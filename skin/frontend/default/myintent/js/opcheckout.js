...
//        this.steps = ['login', 'shipping', 'shipping_method', 'payment', 'billing', 'review'];
...
console.log("checkout.ajaxFailer() - failureURL: " + this.failureUrl);
        location.href = this.failureUrl;
    },

    reloadProgressBlock: function(toStep) {
console.log("checkout.reloadProgressBlock() - toStep: " + toStep);		
	...
	// disables UI buttons before "Ajax" requests
    setLoadWaiting: function(step, keepDisabled) {
console.log("Checkout.setLoadWaiting() start: " + step);							
        if (step) {
console.log("Checkout.setLoadWaiting() in: " + this.loadWaiting);
            if (this.loadWaiting) {
...
console.log("Checkout.setLoadWaiting() out: " + this.loadWaiting);			
            if (this.loadWaiting) {
    ...
    gotoSection: function (section, reloadProgressBlock, close) {
console.log("checkout.gotosection(): " + section);
        if (reloadProgressBlock) {
console.log("checkout.gotosection() - reloadprogressblock: " + this.currentStep);			
            this.reloadProgressBlock(this.currentStep);
        }
        this.currentStep = section;
        var sectionElement = $('opc-' + section);
// customize begin - not all sections will be visible since payment and billing are combined		
		if (sectionElement != null){
// customize end			
			sectionElement.addClassName('allow');
			this.accordion.openSection('opc-' + section, close);
// customize begin			
		}		
// customize end		
        if(!reloadProgressBlock) {
console.log("checkout.gotosection() - noreloadprogressblock");
...
/* customization begin */		
		...
		$("shipping-progress-opcheckout").setStyle({visibility:"visible", position: "static"});
/* customization end */		
...
//        var changeStep = section.replace('opc-', '');
		var changeStep = section.replace('opc-', '');
		$(changeStep + "-progress-opcheckout").setStyle({visibility:"hidden", position: "absolute"});
		$("co-" + changeStep +"-form").show();
/* customization end */				
        this.gotoSection(changeStep, false);
    },
	
    setMethod: function(callback){
console.log("Check.setMethod(): " + callback.checked);
/*	customization begin */
//        if ($('login:guest') && $('login:guest').checked) {
        if (callback.checked == undefined || callback.checked == false) {	
/*	customization end */		
            this.method = 'guest';
...
console.log("Check.setShippingMethod()");		
//        this.nextStep();
...
console.log("checkout.setStepResponse() - response.update_section: " + response.update_section);		
        if (response.update_section) {
            $('checkout-'+response.update_section.name+'-load').update(response.update_section.html);
        }
console.log("checkout.setStepResponse() - response.allow_sections: " + response.allow_sections);		
        if (response.allow_sections) {
            response.allow_sections.each(function(e){
                $('opc-'+e).addClassName('allow');
            });
        }
console.log("checkout.setStepResponse() - response.duplicateBillingInfo: " + response.duplicateBillingInfo);
        if(response.duplicateBillingInfo)
        {
            this.syncBillingShipping = true;
/* customization - not using "Use Billing Address" checkbox in Login step			
            shipping.setSameAsBilling(true);
*/			
        }
console.log("checkout.setStepResponse() - response.goto_section: " + response.goto_section);
        if (response.goto_section) {
            this.gotoSection(response.goto_section, true);
            return true;
        }
console.log("checkout.setStepResponse() - response.redirect: " + response.redirect);		
...
//            }
console.log("billing.save: " + this.saveUrl); //billing.save: http://localhost/index.php/checkout/onepage/saveBilling/
            var request = new Ajax.Request(
                this.saveUrl,
                {
                    method: 'post',
...
/* customization begin */
		...
		$("billing-progress-opcheckout").setStyle({visibility:"visible", position: "static"});
/* customization end */

console.log("billing.nextStep - start ");
        if (transport && transport.responseText){
            try{
                response = eval('(' + transport.responseText + ')');
/* customization begin - need to load payment progress bar since payment is combined into billing */
				checkout.reloadProgressBlock("payment");
/* customization end */	
console.log("billing.nextStep (success): " + response);				
            }
            catch (e) {
                response = {};
            }
        ...
        if (response.error){
console.log("billing.nextStep (error) ");				
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    save: function(){
console.log("Shipping.save() start");		
        if (checkout.loadWaiting!=false) return;
        var validator = new Validation(this.form);
        if (validator.validate()) {
...
console.log("Shipping.nextStep(): " + response);
        checkout.setStepResponse(response);
...
ShippingMethod.prototype = {
    ...
    save: function(){
        ...
        if (checkout.loadWaiting!=false) return;
console.log("Shippingmethod.save() - start");		
        if (this.validate()) {
console.log("Shippingmethod.save() - passed validation");
            checkout.setLoadWaiting('shipping-method');
console.log("Shippingmethod.save() - saveURL: " + this.saveUrl);
console.log("Shippingmethod.save() - onSave: " + this.onSave);
            var request = new Ajax.Request(
                this.saveUrl,
                {
                    method:'post',
...
console.log("Shippingmethod.nextStep() start");		
        if (transport && transport.responseText){
            try{
console.log("Shippingmethod.nextStep() response: " + eval('(' + transport.responseText + ')'));						
                response = eval('(' + transport.responseText + ')');
            }
            catch (e) {
                response = {};
...
//        payment.initWhatIsCvvListeners();
console.log("there");
//console.log("Shippingmethod.nextStep() - response.goto_section: " + response.goto_section);
        if (response.goto_section) {
console.log("in");
            checkout.gotoSection(response.goto_section, true);
//			checkout.reloadProgressBlock(); // doesn't seem necessary
//			executing methods below resulted in "blanking" on info stored in the progress container (i.e. shipping info)
//			checkout.setShippingMethod();
            return;
        }
console.log("Shippingmethod.nextStep() - response.payment_methods_html: " + response.payment_methods_html);			
...
// removed the form for payment fields and included it in billing		
//        if ($(this.form)) {
//            $(this.form).observe('submit', function(event){this.save();Event.stop(event);}.bind(this));
//        }
        var method = null;
...
// fields disabled would be hidden anyways
